...
import { Plus, Github, ArrowRight, Kanban, CheckCircle, Clock, Zap } from "lucide-react";
      ...
      <aside className="w-64 border-r border-border bg-card p-6 flex flex-col justify-between">
        <div>
          <div className="flex items-center gap-3 mb-8">
            <div className="p-2.5 rounded-xl bg-primary text-primary-foreground flex items-center justify-center shadow-lg shadow-primary/20">
              <Zap className="h-6 w-6 text-yellow-400" />
            </div>
            <div>
              <h1 className="font-bold text-lg leading-tight tracking-tight">ShipFlow AI</h1>
              <span className="text-xs text-muted-foreground font-medium">Delivery Engine</span>
            </div>
          </div>

          <nav className="space-y-1.5">
            <Link href="/" className="flex items-center gap-3 px-4 py-2.5 rounded-lg bg-secondary text-secondary-foreground font-medium transition-all">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {projects.map((project) => (
              <Card key={project.id} className="group hover:border-primary/50 transition-all duration-300 rounded-2xl flex flex-col justify-between overflow-hidden relative border-border/80 shadow-md">
                ...
                <CardHeader className="p-6">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2 text-xs font-semibold text-muted-foreground bg-muted py-1.5 px-3 rounded-full border border-border/40">
                ...
                <CardContent className="px-6 py-3 border-t border-border/40 bg-muted/10 flex items-center justify-around text-center">
                  <div className="space-y-1">
                    <span className="text-2xl font-bold block leading-none">0</span>
                    <span className="text-[10px] uppercase font-semibold text-muted-foreground tracking-wider block">Intake</span>
                  </div>
                  <div className="w-[1px] h-8 bg-border/40" />
                  <div className="space-y-1">
                    <span className="text-2xl font-bold block leading-none">0</span>
                    <span className="text-[10px] uppercase font-semibold text-muted-foreground tracking-wider block">Active</span>
                  </div>
                  <div className="w-[1px] h-8 bg-border/40" />
                  <div className="space-y-1">
                    <span className="text-2xl font-bold block leading-none text-emerald-500">0</span>
                    <span className="text-[10px] uppercase font-semibold text-muted-foreground tracking-wider block">Shipped</span>
                  </div>
                </CardContent>
